...
impl<F: Field, const BYTES_IN_FIELD: usize> ToHashBlockCircuitConfig<F, BYTES_IN_FIELD> {
    ...
    pub(crate) fn configure(
        ...
        let is_row_tag_byte = |meta: &mut VirtualCells<F>| {
            and::expr(vec![
                not::expr(meta.query_advice(base_conf.padding, Rotation::cur())),
        ...
        meta.create_gate("always", |meta| {
            ...
            cb.require_boolean("is_field_border", 
        ...
        // current field index is not the last one of the input: i.e PoseidonTable::INPUT_WIDTH
        ...
        meta.create_gate("field byte cycling", |meta| {
            ...
            cb.require_equal("is_field_border := !q_byte_in_field_not_last", 
                meta.query_advice(is_field_border, Rotation::cur()),
        ...
        meta.create_gate("field input cycling", |meta| {
            ...
            cb.require_equal(
                "control_length := base.code_length - bytecode_table.index if q_input_border_last else control_length_prev", 
                meta.query_advice(control_length, Rotation::cur()),
                select::expr(
                    q_input_border_last.clone(),
                    meta.query_advice(base_conf.code_length, Rotation::cur()) - 
        ...
        meta.create_gate("start of bytecode", |meta| {
            ...
            cb.require_equal(
                "control_length := base.code_length - bytecode_table.index",
                meta.query_advice(control_length, Rotation::cur()),
                meta.query_advice(base_conf.code_length, Rotation::cur()) - 
                meta.query_advice(bytecode_table.index, Rotation::cur()),
            );
        ...
        for i in 0..PoseidonTable::INPUT_WIDTH {
            ...
            meta.lookup_any("poseidon input", |meta| {
                // Conditions:
                // - On the row at **field border** (`is_field_border == 1`) or
                ...
                let enable = and::expr(vec![
                    or::expr(vec![
                        meta.query_advice(is_field_border, Rotation::cur()),
                        meta.query_advice(base_conf.is_final, Rotation::cur()),
                    ]),
                    not::expr(meta.query_advice(base_conf.padding, Rotation::cur())),
        ...
        meta.lookup_any("poseidon input padding zero for final", |meta| {
            ...
            for (l_exp, tbl_col) in [
                    ...
                    0.expr(), 
                    meta.query_advice(control_length, Rotation::cur()),
                ].into_iter().zip(pick_hash_tbl_cols(1)) {
                constraints.push((
                    enable.clone() * l_exp,
                    meta.query_advice(tbl_col, Rotation::cur()),
                ))
    ...
    pub(crate) fn assign(
        ...
        assert!(size > self.base_conf.minimum_rows);
        let mut is_first_time = true;

        layouter.assign_region(
            || "assign bytecode and field block extension",
            |mut region| {
                is_first_time = false;
                self.base_conf.assign_internal(&mut region, size, witness, challenges, true, is_first_time)?;
                self.assign_internal(&mut region, size, witness, true)
            }
        )
    ...
    pub(crate) fn assign_internal(
        ...
        for bytecode in witness.iter() {
            ...
            offset += bytecode.rows.len();
        }

        let last_row_offset = size - self.base_conf.minimum_rows + 1;
        // additional Padding
        for idx in offset..= last_row_offset {
            self.assign_blank_row(region, idx)?;
        }
        Ok(())
    }

    /// Assign a blank row (at padding or start line of each bytecodes)
    pub(crate) fn assign_blank_row(
        &self, 
        region: &mut Region<F>, 
        offset: usize,
    ) -> Result<(), Error> {
        for (name, column) in [
            ...
            ("field input blank", self.field_input),
            ...
            ("padding shift blank", self.padding_shift),
        ...
        match tag {
            i if i == BytecodeFieldTag::Byte as u32 => {
                ...
                for (tip, column, val) in [
                    ("control length", self.control_length, F::from(control_length as u64)),
                    ("field input", self.field_input, input_f),
                    ("bytes in field", self.bytes_in_field_index, F::from(bytes_in_field_index as u64)),
                    ("bytes in field inv", self.bytes_in_field_inv, bytes_in_field_index_inv_f),
                    ("field border", self.is_field_border, F::from(field_border as u64)),
            ...
            i if i == BytecodeFieldTag::Length as u32 => {
                ...
                self.assign_blank_row(region, offset)?;
                for (name, column, val) in [
                    ("control length header", self.control_length, F::from(code_length as u64)),
                    ("padding shift header", self.padding_shift, F::from(256 as u64).pow_vartime(&[BYTES_IN_FIELD as u64])),
                    ("field index header", self.field_index, F::one()),
                ] {
                    region.assign_advice(
                        || format!("assign header{} {}", name, offset),
                        column,
                        offset,
                        || Value::known(val),
                    )?;
